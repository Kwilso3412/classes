...
class apriori_algo:
    ...
    # Following functions are for preprocessing
    '''
    ...
    '''
    def bool_converter(dataframe, column, threshold):
        dataframe[column] = dataframe[column].apply(lambda x: True if x >= threshold else False)
    ...
    def one_hot_selective(self, dataframe, columns_to_convert, column_to_pop_out):
        ...
        for i, row in columns_to_convert.iterrows():
            num_col[row['column']] = self.bool_converter(num_col, row['column'], row['threshold'])

        encoded_cat_cols = pd.get_dummies(cat_col).astype(bool)
